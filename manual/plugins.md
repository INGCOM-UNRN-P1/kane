---
title: "Guía de Extensión y Creación de Plugins: kane"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de kane"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-kane-plugins)=
# Guía de Extensión y Plugins: kane

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`kane`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-kane-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`kane` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para kane`.
- **Punto de Entrada Oficial**: `kane.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-kane-plugins-tutorial)=
...
Para que `kane` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."kane.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `kane` la reconozca:
...
(manual-kane-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `kane` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = kane.ejecutar_analisis(
...
(manual-kane-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `kane` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar kane
        run: pip install -e ./kane
        
      - name: Ejecutar Auditoría
        run: kane check src/ include/ --json > reporte.json
````

---

(manual-kane-plugins-ejercicios)=
